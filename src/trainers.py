...
from utils import recall_at_k, ndcg_k, cal_mrr, get_metric
    ...
    def iteration(self, epoch, dataloader, cluster_dataloader=None, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch) in rec_cf_data_iter:
                ...
                # ---------- prediction task -----------------------#
                intent_output, diff_loss = self.model(subsequence_1)
                ...
                # ---------- multi-task learning --------------------#
                joint_loss =self.args.rec_weight * rec_loss + icl_loss + self.args.diff_loss * diff_loss  # 推荐的损失 + 意图损失（为 0 ） + 扩散模型损失
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    recommend_output, _ = self.model(input_ids) # [BxLxH]
